...
def compress(input_file, output_file):
    ...
    for byte in data:
        ...
        if closest_node and abs(closest_node.value - byte) <= closest_node.error_threshold * 256:
            bit_stream.append(1)  # No error
        else:
            bit_stream.append(0)  # Error
...
def decompress(input_file, output_file):
    ...
    error_index = 0
    byte = 0
    print(f"Bit stream length: {len(bit_stream)}, Error stream length: {len(error_stream)}")
    ...
    for i, bit in enumerate(bit_stream):
        if bit:  # No error
            ...
            print(f"Bit {i}: Predicted byte {byte}")
        else:  # Error
            if error_index < len(error_stream):
                byte = error_stream[error_index]
                error_index += 1
                print(f"Bit {i}: Error byte {byte}")
        
        output.append(byte)
        tree.insert(byte)
        
        if i % 1000 == 0:
            print(f"Processed {i} bits, current output size: {len(output)} bytes")
    
    with open(output_file, 'wb') as f:
        f.write(output)

    print(f"Decompressed data written to {output_file}")
    print(f"Final output size: {len(output)} bytes")
